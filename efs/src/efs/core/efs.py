...
class EFS(BaseEstimator, RegressorMixin):
    """
    Optimized Ensembled Forward Selection (EFS) for feature selection.
    ...
    def fit(self, X, y):
        """
        Fit EFS with optimized implementation.
...
class EFSCV(BaseEstimator, RegressorMixin):
    """
    Cross-validation wrapper for EFS with custom scoring support.
