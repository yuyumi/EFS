...
def _run_single_simulation_worker(args):
    ...
    try:
        ...
        from efs_experiments.simulation.data.data_generator import DataGenerator
        from efs_experiments.simulation.orchestration.experiment_orchestrator import ExperimentOrchestrator
        from efs_experiments.simulation.config.parameter_loader import get_sigma_list
        from tqdm import tqdm
...
class SimulationPipeline:
    ...
    def _create_summary_statistics(self, results_df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary_metrics = {
            ...
            'best_m': ['mean', 'std'],
            'best_k': ['mean', 'std'],
            'best_k_bagged_fs': ['mean', 'std'],
            'best_k_smeared_fs': ['mean', 'std'],
            'best_k_fs': ['mean', 'std'],
            ...
            # BaggedFS metrics
            'insample_bagged_fs': ['mean', 'std'],
            'mse_bagged_fs': ['mean', 'std'],
            'df_bagged_fs': ['mean', 'std'],
            'coef_recovery_bagged_fs': ['mean', 'std'],
            'support_recovery_bagged_fs': ['mean', 'std'],
            'outsample_mse_bagged_fs': ['mean', 'std'],
            'rte_bagged_fs': ['mean', 'std'],
            'f_score_bagged_fs': ['mean', 'std'],
            
            # SmearedFS metrics
            'insample_smeared_fs': ['mean', 'std'],
            'mse_smeared_fs': ['mean', 'std'],
            'df_smeared_fs': ['mean', 'std'],
            'coef_recovery_smeared_fs': ['mean', 'std'],
            'support_recovery_smeared_fs': ['mean', 'std'],
            'outsample_mse_smeared_fs': ['mean', 'std'],
            'rte_smeared_fs': ['mean', 'std'],
            'f_score_smeared_fs': ['mean', 'std'],
            
            # EFSCV metrics
            'insample_efs': ['mean', 'std'],
            'mse_efs': ['mean', 'std'],
            'df_efs': ['mean', 'std'],
            'coef_recovery_efs': ['mean', 'std'],
            'support_recovery_efs': ['mean', 'std'],
            'outsample_mse_efs': ['mean', 'std'],
            'rte_efs': ['mean', 'std'],
            'f_score_efs': ['mean', 'std'],
            
            # Forward Selection metrics
            'insample_fs': ['mean', 'std'],
            'mse_fs': ['mean', 'std'],
            'df_fs': ['mean', 'std'],
            'coef_recovery_fs': ['mean', 'std'],
            'support_recovery_fs': ['mean', 'std'],
            'outsample_mse_fs': ['mean', 'std'],
            'rte_fs': ['mean', 'std'],
            'f_score_fs': ['mean', 'std']
        }
    ...
    def _create_timing_summary(self, results_df: pd.DataFrame) -> pd.DataFrame:
        ...
        timing_metrics = {
            ...
            'time_bagged_fs': ['mean', 'std', 'min', 'max'],
            'time_smeared_fs': ['mean', 'std', 'min', 'max'],
            'time_efscv': ['mean', 'std', 'min', 'max'],
            'time_original_fs': ['mean', 'std', 'min', 'max']
        }
    ...
    def run_full_simulation(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if 'rte_lasso' in summary.columns:
            ...
            for model in ['lasso', 'bagged_fs', 'efs']:
