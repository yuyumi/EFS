...
**Supported Models**:
- **BaggedFS**: Bagged Forward Selection with cross-validation
- **SmearedFS**: Data Smearing with Forward Selection 
- **EFSCV**: Ensembled Forward Selection with cross-validation
- **Forward Selection**: Standard greedy selection (EFSCV with specific parameters)

**Key Methods**:
- `create_bagged_fs(seed, sim_num)`: Creates a BaggedFS model
- `create_smeared_fs(seed, sim_num)`: Creates a SmearedFS model
- `create_efscv(seed, sim_num)`: Creates an EFSCV model
- `create_forward_selection(seed, sim_num)`: Creates a Forward Selection model
...
**Key Methods**:
- `extract_coefficients_regular(model)`: Extracts coefficients from regular models (EFSCV)
