...
def calculate_df_for_all_k(model: Any, 
    ...
    """
    Calculate degrees of freedom for each k value in a fitted EFS model.
    ...
    """
    # For EFSCV models, extract the underlying EFS model
    if hasattr(model, 'model_'):
        efs_model = model.model_
    else:
        efs_model = model
    ...
    for k in range(len(efs_model.coef_)):
        # Get predictions for this k
        y_pred_k = efs_model.predict(X_train, k=k)
