...
├── _fit_rfs_models()             # BaggedFS, SmearedFS, EFSCV, FS
...
print(f"EFSCV RTE: {results['rte_efs']:.3f}")
print(f"BaggedFS best k: {results['best_k_bagged_fs']}")
```
...
```python
{
    ...
    # Model performance (for each model: lasso, ridge, elastic, bagged_fs, smeared_fs, efs, fs)
    ...
    'best_m': optimal_m_value,  # for EFSCV
    'best_noise_scale': optimal_noise_scale,  # for SmearedFS
