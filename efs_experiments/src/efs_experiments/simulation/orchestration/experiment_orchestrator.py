...
class ExperimentOrchestrator:
    ...
    def _fit_rfs_models(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        """
        Fit and evaluate RFS models (BaggedFS, SmearedFS, EFSCV, Forward Selection).
        ...
        # Fit BaggedFS
        bagged_model, start_time = model_factory.create_bagged_fs(seed, sim_num)
        bagged_model, fitting_time = model_factory.fit_model(bagged_model, X_train, y_train, start_time)
        timing_results['time_bagged_fs'] = fitting_time
        ...
        bagged_results = model_evaluator.evaluate_model_complete(
            ...
            model_name="bagged_fs", is_ensemble=True
        )
        results.update(bagged_results)
        
        # Fit SmearedFS
        smeared_model, start_time = model_factory.create_smeared_fs(seed, sim_num)
        smeared_model, fitting_time = model_factory.fit_model(smeared_model, X_train, y_train, start_time)
        timing_results['time_smeared_fs'] = fitting_time
        ...
        smeared_results = model_evaluator.evaluate_model_complete(
            ...
            X_test=X_test, y_test=y_test, beta_true=beta_true, cov_matrix=cov_matrix,
            model_name="smeared_fs", is_ensemble=True
        )
        results.update(smeared_results)
        
        # Fit EFSCV
        efscv_model, start_time = model_factory.create_efscv(seed, sim_num)
        efscv_model, fitting_time = model_factory.fit_model(efscv_model, X_train, y_train, start_time)
        timing_results['time_efscv'] = fitting_time
        
        efscv_results = model_evaluator.evaluate_model_complete(
            model=efscv_model, X_train=X_train, y_train=y_train, y_true_train=y_true_train,
            X_test=X_test, y_test=y_test, beta_true=beta_true, cov_matrix=cov_matrix,
            model_name="efs", is_ensemble=False
        )
        results.update(efscv_results)
        
        # Fit Forward Selection
        gs_model, start_time = model_factory.create_forward_selection(seed, sim_num)
        gs_model, fitting_time = model_factory.fit_model(gs_model, X_train, y_train, start_time)
        timing_results['time_original_fs'] = fitting_time
        ...
        gs_results = model_evaluator.evaluate_model_complete(
            ...
            X_test=X_test, y_test=y_test, beta_true=beta_true, cov_matrix=cov_matrix,
            model_name="fs", is_ensemble=False
        )
