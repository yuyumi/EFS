...
from efs import EFS  # Import EFS class

class BaggedFS(BaseEstimator, RegressorMixin):
    """
    Bagged Forward Selection with CV for optimal k.
    Uses EFS with m=p and B=1 for each bootstrap sample.
    ...
    def _fit_individual_fs(self, X, y, bootstrap=True, random_state=None):
        """Fit a single FS model using EFS with m=p and B=1."""
        n, p = X.shape
        ...
        # Use EFS with m=p (all features) and B=1 (one replicate)
        efs = EFS(
            k_max=self.k_max,
        ...
        efs.fit(X_sample, y_sample)
        
        # Extract coefficients and intercepts for all k values
        coefs = np.array([coef for coef in efs.coef_])
        intercepts = np.array([intercept for intercept in efs.intercept_])
    ...
    def fit(self, X, y):
        ...
        if self.cv == 1:
            ...
            for i in range(self.n_estimators):
                ...
                coefs, intercepts, features = self._fit_individual_fs(
